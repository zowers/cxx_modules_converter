...
class ModuleBaseBuilder(FileBaseBuilder):
    module_purview_start_prefix: str = (
        ''  # 'module' or 'export module' - overriden in implementation classes
    )
    ...
    def __init__(
        ...
        self.resolver: ModuleFilesResolver = ModuleFilesResolver(
            self.parent_resolver, self.options
        )
        ...
        self.global_module_fragment_compat_includes: StrList = (
            []
        )  # compat header includes
        self.global_module_fragment_compat_end: StrList = (
            []
        )  # compat header includes endif
        self.global_module_fragment: StrList = []  # header includes
        self.module_purview_start: StrList = (
            []
        )  # export module <name>; // Start of module purview.
        # self.module_imports: StrList = []
        # self.module_purview_special_headers: StrList = [] # // Configuration, export, etc.  # noqa: E501
    ...
    def set_source_filename(self, source_filename: Path):
        ...
        module_name = self.parent_resolver.make_defined_module_for_path(
            pure_source_filename
        )
        if not module_name:
            module_name = self.parent_resolver.convert_filename_to_module_name(
                pure_source_filename
            )
        if not module_name:
            raise ConversionError(
                f"Failed to determine module name for file {pure_source_filename}"
            )
    ...
    def set_module_purview_start(self):
        ...
        module_purview_start = (
            f'''{self.module_purview_start_prefix} {self.module_name};'''
        )
    ...
    def handle_main_content(self, line: str):
        self.processing_state.flush_module_staging(
            self, self._set_main_module_content_start
        )
    ...
    def convert_as_compat_header(self):
        return (
            self.file_options.convert_as_compat
            and self.content_type == ContentType.MODULE_INTERFACE
        )
    ...
    def add_module_import_from_include(
        self, line: str, match: re.Match[str] | None = None, is_quote: bool = False
    ):
        self.set_module_purview_start()
        if not match:
        ...
        resolved_include_filename = self.resolver.resolve_include(
            line_include_filename, is_quote
        )
        if resolved_include_filename is None and is_quote:
            self.add_global_module_fragment(line)
            return
        if any_pattern_maches(
            self.options.always_include_names,
            resolved_include_filename or PurePosixPath(line_include_filename),
        ):
            self.add_global_module_fragment(line)
            return

        line_module_name = self.resolver.resolve_include_to_module_name(
            line_include_filename, is_quote, False
        )
        if line_module_name is None:
            self.add_global_module_fragment(line)
            return
        full_line_module_name = self.resolver.resolve_include_to_module_name(
            line_include_filename, is_quote, True
        )
        ...
        export_opt = (
            '''export ''' if self._needs_export_for_import(line_module_name) else ''
        )

        import_line = f'''{line_space1}{line_space2}{export_opt}import {line_module_name};{line_tail}'''  # noqa: E501
    ...
    def _needs_export_for_import(self, import_module_name: str) -> bool:
        ...
        if owner_exports and (
            import_module_name in owner_exports or STAR_MODULE_EXPORT in owner_exports
        ):
            return True
        star_owner_exports = self.options.export.get(STAR_MODULE_EXPORT)
        if star_owner_exports and (
            import_module_name in star_owner_exports
            or STAR_MODULE_EXPORT in star_owner_exports
        ):
            return True
    ...
    def build_result(self):
        if self.global_module_fragment or self.global_module_fragment_compat_includes:
            assert (
                bool(self.global_module_fragment_start) == self.get_is_actually_module()
            )
        module_interface_builder = self.get_module_interface_builder()
        if (
            self.content_type == ContentType.MODULE_IMPL
            and self.get_is_actually_module()
            and module_interface_builder
            and module_interface_builder.global_module_fragment
        ):
            # include interface GMF at the start of impl GMF
            self.set_global_module_fragment_start()
            self.global_module_fragment = (
                module_interface_builder.global_module_fragment
                + self.global_module_fragment
            )
        self.processing_state.flush_module_staging(self)
