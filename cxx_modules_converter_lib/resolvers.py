        ...
        for root, dirs, files in os.walk(path):
            ...
            if relative_root == Path(''):
                ...
                if not isinstance(parent_node, dict):
                    raise RuntimeError(
                        f"Expected dict for parent node, got {type(parent_node).__name__}"  # noqa: E501
                    )
...
class FilesResolver:
    ...
    def resolve_in_search_path(
        ...
        if is_quote:
            logging.warning(
                f'file not found: "{include_filename}" referenced from "{current_filename}"'  # noqa: E501
            )
            return include_path
        return None
    ...
    def filename_to_module_name(
        self, filename: PurePosixPath, base_module: str | None
    ) -> str:
        ...
        for pattern, target_module in self.options.join_configurations.items():
            if fnmatch.fnmatchcase(filename.as_posix(), pattern):
                ...
                if is_star_pattern or is_slash_star_pattern:
                    ...
                    if is_star_pattern or filename.as_posix().startswith(
                        pattern_dir + '/'
                    ):
    ...
    def get_source_content_type(
        self, action: ConvertAction, filename: Path
    ) -> ContentType:
    ...
    def convert_filename_to_content_type(
        self, filename: Path, content_type: ContentType
    ) -> str:
        parts = os.path.splitext(filename)
...
class ModuleFilesResolver:
    ...
    def resolve_include(
        self, include_filename: str, is_quote: bool
    ) -> PurePosixPath | None:
        result = self.parent_resolver.resolve_in_search_path(
            self.module_dir, self.module_filename, include_filename, is_quote
        )
        return result

    def resolve_include_to_module_name(
        self, include_filename: str, is_quote: bool, use_full_name: bool = False
    ) -> str | None:
        resolved_include_filename = self.parent_resolver.resolve_in_search_path(
            self.module_dir, self.module_filename, include_filename, is_quote
        )
        if resolved_include_filename is None:
            result = self.parent_resolver.make_defined_module_for_path(
                PurePosixPath(include_filename)
            )
            return (
                self.get_module_name_for_import(result, use_full_name)
                if result
                else result
            )
        result = self.parent_resolver.make_defined_module_for_path(
            resolved_include_filename
        )
        if result is not None:
            return self.get_module_name_for_import(result, use_full_name)
        result = self.parent_resolver.convert_filename_to_module_name(
            resolved_include_filename
        )
        return self.get_module_name_for_import(result, use_full_name)
