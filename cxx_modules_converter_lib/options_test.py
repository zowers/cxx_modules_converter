...
def test_add_join_configuration_star():
    ...
    result = files_resolver.filename_to_module_name(
        PurePosixPath('subdir/test.h'), None
    )
...
def test_set_root_dir_module_name_and_add_join_configuration():
    ...
    result1 = converter.resolver.convert_filename_to_module_name(
        PurePosixPath('subdir/test.h')
    )
    assert result1 == 'org.mymodule:test'

    result2 = converter.resolver.convert_filename_to_module_name(
        PurePosixPath('other/test.h')
    )
