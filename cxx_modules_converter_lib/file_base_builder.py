...
class FileBaseBuilder:
    ...
    def converted_filename(self) -> str:
        return self.parent_resolver.convert_filename_to_content_type(
            self.source_filename, self.content_type
        )
...
class FileProcessingState:
    def __init__(self):
        ...
        self.flushed_module_preprocessor_nesting_count: int = (
            0  # count of opened preprocessor #if statements flushed to module content
        )
        self.global_module_fragment_staging: StrList = (
            []
        )  # staging area for next global_module_fragment entry
        self.preprocessor_nesting_count: int = (
            0  # count of opened preprocessor #if statements
        )
        self.global_module_fragment_includes_count: int = (
            0  # count of #include <> statements
        )
        self.flushed_global_module_fragment_includes_count: int = (
            0  # count of #include <> statements flushed to global module content
        )
        self.module_staging_last_unnested_index: int = (
            0  # last index in module staging without opened preprocessor #if statements
        )

    def add_module_staging(self, line: str, nesting_advance: int):
        if (
            self.preprocessor_nesting_count == 0
            and nesting_advance == 0
            or self.preprocessor_nesting_count == 1
            and nesting_advance == 1
        ):
    ...
    def add_global_module_fragment_staging(
        self, builder: 'ModuleBaseBuilder', line: str, nesting_advance: int
    ):
        self.global_module_fragment_staging.append(line)
        self.flush_global_module_fragment(builder)

    def flush_module_staging(
        self,
        builder: 'ModuleBaseBuilder',
        last_unnested_inserter: Callable[[], None] | None = None,
    ):
        builder.set_module_purview_start()
        if (
            self.flushed_global_module_fragment_includes_count == 0
            or self.flushed_module_preprocessor_nesting_count != 0
        ):
            for i in range(len(self.module_staging)):
                if (
                    last_unnested_inserter
                    and i == self.module_staging_last_unnested_index
                ):
                    last_unnested_inserter()
            ...
            if (
                last_unnested_inserter
                and self.module_staging_last_unnested_index == len(self.module_staging)
            ):
                last_unnested_inserter()
        self.module_staging = []
    ...
    def flush_global_module_fragment(self, builder: 'ModuleBaseBuilder'):
        ...
        if (
            ...
            and not builder.convert_as_compat_header()
        ):
            return
        ...
        self.flushed_global_module_fragment_includes_count = (
            self.global_module_fragment_includes_count
        )
        self.global_module_fragment_includes_count = 0
