...
class CompatHeaderBuilder(FileBaseBuilder):
    ...
    def __init__(self, options: Options, module_builder: ModuleBaseBuilder):
        ...
        if module_builder.content_type != ContentType.MODULE_INTERFACE:
            raise ConfigurationError(
                f"CompatHeaderBuilder requires MODULE_INTERFACE builder, "
                f"got {module_builder.content_type}"
            )
        ...
        self.relative_module_interface_unit_filename: str = os.path.basename(
            module_interface_unit_filename
        )
    ...
    def build_result(self) -> str:
        ...
        relative_module_interface_unit_filename = (
            self.relative_module_interface_unit_filename
        )
        parts = [
            f'''#pragma once''',
